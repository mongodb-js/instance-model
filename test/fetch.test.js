...
var fixtures = require('mongodb-connection-fixture').MATRIX.map(function(
  model
) {
...
describe('mongodb-instance-model#fetch', function() {
  describe('local', function() {
    ...
    it('should not close the db after getting instance details', function(
      done
    ) {
      assert(db);
  ...
  it.skip('should get instance details for john doe', function(done) {
    var connection = Connection.from(
  ...
  if (fixtures.length > 0) {
    describe('functional #slow', function() {
      _.map(_.groupBy(fixtures, 'authentication'), function(
        models,
        authentication
      ) {
        describe(
          format('Using authentication `%s`', authentication),
          function() {
