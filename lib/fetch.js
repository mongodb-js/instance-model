...
function getBuildInfo(done, results) {
  ...
  db.db('admin').command(spec, options, (err, res) => {
    if (err) {
...
function getDatabase(db, name, done) {
  ...
  db.db(name).command(spec, options, (err, res) => {
    if (err) {
...
function getDatabases(done, results) {
  ...
  async.parallel(_.map(dbnames, (name) => {
...
function getAllowedCollections(done, results) {
  ...
  const collections = security.getResourcesWithActions(
...
function getDatabaseCollections(db, done) {
  ...
  db.listCollections({}, options).toArray(function(err, res) {
    if (err) {
      if (isNotAuthorized(err)) {
        // if the error is that the user is not authorized, silently ignore it
