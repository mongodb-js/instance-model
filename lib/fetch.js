...
function listDatabases(done, results) {
  var db = results.db;
  var userPrivileges = results.userPrivileges;

  var cluster = security.getResourcesWithActions(
    userPrivileges, ['listDatabases'], 'special').filter(function(resource) {
...
function getUserPrivileges(done, results) {
  var db = results.db;
...
function getAllowedDatabases(done, results) {
  ...
  var databases = security.getResourcesWithActions(
    userPrivileges, ['listCollections'], 'database').map(function(resource) {
      return resource.db;
    });
...
function getAllowedCollections(done, results) {
  ...
  var collections = security.getResourcesWithActions(
    userPrivileges, compassActions, 'collection').map(function(resource) {
      return {
        db: resource.db,
...
function getInstanceDetail(db, done) {
  var tasks = {
    ...
    userPrivileges: ['db', getUserPrivileges],

    host: ['db', getHostInfo],
    build: ['db', getBuildInfo],

    listDatabases: ['db', 'userPrivileges', listDatabases],
    allowedDatabases: ['userPrivileges', getAllowedDatabases],
    databases: ['db', 'listDatabases', 'allowedDatabases', getDatabases],

    listCollections: ['db', 'databases', listCollections],
    allowedCollections: ['userPrivileges', getAllowedCollections],
  ...
  async.auto(tasks, function(err, results) {
    ...
    results = _.omit(results, ['db', 'listDatabases', 'allowedDatabases',
      'userPrivileges', 'listCollections', 'allowedCollections']);
